<?php
    ...
    //if this page have nurse as post add this will be execu
    if($_POST["add"]== "nurse")
    {
        // error check to see all of post information passed to this pahe
        // print_r($_POST);
        
            //set up all of my varibles that I plan to use that were passed in post
        ...
        //varribles created to check so see if everything required was filled out
        $count=0; //adds up if requireements not meet 
        $error=0;  // adds up for errors
        
        //array to hold errors so they can be sent back to employee page
        $arr= array(); 
        
        //these fields are Required to database -fname,lname, specality, and license (count) 
        //i would say email is required technically because emial must have . + @
        
        //make sure name is not empy, null, or greater than 255 length 
        if( $fname =="" || $fname == NULL || strlen($fname)>255)
        {
             ...
             $arr["fname"] = "Error-first name is required";
            ...
            if(strlen($lname)>255)
             {
              ...
              $arr["lname"] = "Error-last name is required";
        ...
        {
             $arr["license"] = "Error-license is required";
             $count++; 
        }
        ...
        if ($s1 =="" || $s1 == NULL)
        {
           $arr["rnspecality"] = "Error-primary specality is required";
           $count++; 
        }
        ...
        if(!$at== 2)
        {
            $arr["email"] = "Error-email invalid must conatin @ and .";
            $error++; 
        }   
        ...
        {   
            ...
            // make suee too add feature that wont allow double entries
            
            
            //inputs to RN_HCP
        if(!($stmt = $mysqli->prepare("INSERT INTO RN_HCP 
           (title, fname, lname, cellphone, homephone, email,
            state, city, adress, recruiter) VALUES ('$title', '$fname', 
		    ...
		    echo "you have Inserted the Health care Professional(RN): ".$fname." ".$lname;  
      ...
      // takes us back to the java script because ($_POST["add"]== "nurse")
        ...
        $holder = array ($top, $state, $topcity, $topnumber); 
            
        $key = array("title", "state", "city"); 
            
            $check=0; 
            for($x = 0; $x< 3;$x++)
            {
                if ($holder[$x] == NULL || $holder[$x]== " ")
                {
                    $check++;
                    echo "$key[$x] must have a value<br>"; 
                }
            }
            ...
            if ($check==0)
            {
                ...
                // echo "connection worked! <br> ";
                 ...
                 if(!($stmt = $mysqli->prepare("UPDATE RN_top SET jtitle= '$top', jlocation='$place', jtype='$toptype' WHERE id = '$topnumber'")))
                            {
                                echo "Prepare failed (UPDATE):
                                    (" . $mysqli->errno . ") " . $mysqli->error;
                            }
	                       if(!$stmt->execute()) 
                           {
                                echo "Execute failed (UPDATE):
                                    (" . $mysqli->errno . ") " . $mysqli->error;
	                       }  
                
                        echo" $top in $place has been added"; 
            }            
        }

        if($_POST["add"]== "job")
        {
            //not as detailed in regard to error catching as the rn due to time constrainst
            
          //  print_r($_POST);
           // echo "<br>you wante to add a job<br>";
            
            $hourly =$_POST["hourly"];
            $salary = $_POST["salary"]; 
		    $position = $_POST["position"];  
            $typeofemployement = $_POST["typeofemployeement"];
            $shift = $_POST["shift"];
            ...
            //these fields are Required to database -fname, -fstate, -fcity, -dname, -position, 
        ...
        if( $fname =="" || $fname == NULL || strlen($fname)>255)
        {
             ...
             { 
                 //too long to hold in datadbase 
                 $arr["fnamelength"] = "Error-facility name is too long to put in DB"; 
                 //add to keep track of errors
                 $error++; 
             }
             $arr["fname"] = "Error- facility name is required";
             //add to keep track of all requirements not met
             $count++; 
        }
        ...
        if ( $fstate =="" || $fstate== NULL)
         {
              $arr["fstate"] = "Error- facility state is required";
             // //add to keep track of all requirements met
             $count++; 
         }
        
        //make sure that at least one license has been selected
        if ( $fcity =="" || $fcity == NULL|| strlen($fcity)>255)
        {
            if(strlen($fname)>255)
             { 
                 //too long to hold in datadbase 
                 $arr["fcitylength"] = "Error-facility city is too long to put in DB"; 
                 //add to keep track of errors
                 $error++; 
             } 
            else 
            {
                $arr["fcity"] = "Error- facility city is required";
                $count++;
            }
        }
        
        //make sure that specality is selected
        if( $dname =="" || $dname == NULL || strlen($dname)>255)
        {
            if(strlen($dname)>255)
             { 
                 //too long to hold in datadbase 
                 $arr["dnamelength"] = "Error-facility name is too long to put in DB"; 
                 //add to keep track of errors
                 $error++; 
             }
            
                $arr["dname"] = "Error- debarment name is required";
                $count++;
        
        }
        
         if( $position =="" || $postion == NULL || strlen($position)>255)
        {
             if(strlen($position)>255)
             { 
                 //too long to hold in datadbase 
                 $arr["postionlength"] = "Error-position name is too long to put in DB"; 
                 //add to keep track of errors
                 $error++; 
             }
             else 
             {
                $arr["position"] = "Error- position name is required";
                //add to keep track of all requirements not met
                $count++;
             }
        }
        ...
        {   
            ...
            echo "put the job in the database<br>"; 
        }
